...
class NeuralNetwork(object):
  ...
  def avepool2d(self, input, scope_name, pool_shape=[1, 2, 2, 1], strides=[1, 2, 2, 1]):
    with tf.variable_scope(scope_name):
      out = tf.layers.average_pooling2d(input,
                           pool_size=(2,2),
                           name="out",
                           padding="SAME",
                           strides=(2,2))
...
class ConvEncoderNew(NeuralNetwork):
  def __init__(self, input_shape, keep_prob, scope_name="encoder_new"):
    self.input_shape = input_shape
    self.keep_prob = keep_prob
    self.scope_name = scope_name
  ...
  def build_graph(self, input):
    ...
    with tf.variable_scope(self.scope_name):
      conv1 = self.conv2d_relu(input, filter_shape=[3, 3, 1, 8], scope_name="conv1")  # (232, 196, 8)
      pool1 = self.avepool2d(conv1, scope_name="pool1")  # (116, 98, 8)
      drop1 = self.dropout(pool1, keep_prob=self.keep_prob, scope_name="drop1")
      conv2 = self.conv2d_relu(drop1, filter_shape=[5, 5, 8, 16], scope_name="conv2")  # (116, 98, 16)
      pool2 = self.avepool2d(conv2, scope_name="pool2")  # (58, 49, 16)
      drop2 = self.dropout(pool2, keep_prob=self.keep_prob, scope_name="drop2")
      drop2 = tf.reshape(drop2, shape=[-1, 58*49*16])  # (45472,)
      fc1 = self.fc(drop2, output_shape=1024, scope_name="fc1")
      drop3 = self.dropout(fc1, keep_prob=self.keep_prob, scope_name="drop3")
...
class DeconvDecoderNew(NeuralNetwork):
  def __init__(self, keep_prob, output_shape, scope_name="decoder_new"):
    self.keep_prob = keep_prob
    self.output_shape = output_shape
    self.scope_name = scope_name
...
class ConvEncoderUNet(NeuralNetwork):
  def __init__(self, input_shape, keep_prob, scope_name="encoder_unet"):
    self.input_shape = input_shape
    self.keep_prob = keep_prob
    self.scope_name = scope_name
  ...
  def build_graph(self, input):
    with tf.variable_scope(self.scope_name):
      ...
      pool3 = self.maxpool2d(conv6, scope_name="pool3")
...
class DeconvDecoderUNet(NeuralNetwork):
  def __init__(self, keep_prob, output_shape, scope_name="decoder_unet"):
    self.keep_prob = keep_prob
    self.output_shape = output_shape
    self.scope_name = scope_name
...
#     with tf.variable_scope(self.scope_name):
#       conv1 = self.conv2d_relu(input, filter_shape=[3, 3, 1, 64], scope_name="conv1")  # (b, 232, 196, 64)
#       deconv2 = self.deconv2d(conv1, filter_shape=[2, 2], num_outputs=64, scope_name="deconv2")  # (b, 232, 196, 64)
#       out = tf.identity(deconv2, name="out")
...
class ConvEncoderDeepUNet(NeuralNetwork):
  def __init__(self, input_shape, keep_prob, scope_name="encoder_unet_deep"):
    self.input_shape = input_shape
    self.keep_prob = keep_prob
    self.scope_name = scope_name
...
class DeconvDecoderDeepUNet(NeuralNetwork):
  def __init__(self, keep_prob, output_shape, scope_name="decoder_unet_deep"):
    self.keep_prob = keep_prob
    self.output_shape = output_shape
    self.scope_name = scope_name
...
class ConvEncoderShallowUNet(NeuralNetwork):
  def __init__(self, input_shape, keep_prob, scope_name="encoder_unet_shallow"):
    self.input_shape = input_shape
    self.keep_prob = keep_prob
    self.scope_name = scope_name
...
class DeconvDecoderShallowUNet(NeuralNetwork):
  def __init__(self, keep_prob, output_shape, scope_name="decoder_unet_shallow"):
    self.keep_prob = keep_prob
    self.output_shape = output_shape
    self.scope_name = scope_name
